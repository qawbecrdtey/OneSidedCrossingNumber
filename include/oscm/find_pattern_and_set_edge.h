...
namespace oscm {
     ...
     * @param connections_ Edges as pair of vertices.
     * @param directed_edges_ will be updated to contain directed edges from B to B, labeled as v -
     * nA_.
     */
    __attribute__((flatten)) inline void find_pattern_and_set_edge(
            ...
            for(std::uint32_t j = i + 1; j != m; j++) {
                ...
                if(Cij == 1 && Cji == 2) {
                    auto const it_lo_i = std::lower_bound(
                      connections_.begin(), connections_.end(), std::make_pair(0u, i), comp_second);
                    auto const it_hi_i = std::upper_bound(
                      connections_.begin(), connections_.end(), std::make_pair(0u, i), comp_second);
                    if(it_hi_i - it_lo_i != 2) { continue; }
                    auto const it_lo_j = std::lower_bound(
                      connections_.begin(), connections_.end(), std::make_pair(0u, j), comp_second);
